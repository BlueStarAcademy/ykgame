...
또는 큰 쿠키 배열은 파일로 전달한다(권장, Windows env 크기 한도 회피):

```text
K6_SESSION_COOKIES_JSON_FILE=scripts/load/.staging-cookies.json
```

`scripts/create-staging-load-cookies.mjs`로 staging 전용 계정을 만들고
쿠키 파일을 생성할 수 있다. 파일은 커밋하지 않는다.
...
`REDIS_URL`이 없으면 dump rate limit만 제한적 fail-open한다(로컬 개발용).
`REDIS_URL`이 설정됐는데 Redis가 장애이면 dump는 **fail-closed(429)** 로
Postgres 쓰기 폭주를 막고, 구조화 API 로그의 `rateLimitBypassed=true`로
식별한다. TOP10 랭킹(20초)·사용자 stats(20초)·시즌 drop coupon 집계(5초)·
sessionVersion(15초)은 Redis 공유 캐시를 사용하고, Redis를 쓸 수 없을 때만
프로세스 로컬 TTL 캐시로 fallback한다. score 저장 성공 시 해당 game/season
TOP10과 사용자 stats key를 지우며 duplicate replay는 무효화하지 않는다.
...
1. Redis 기능이 원인이면 웹 서비스의 `REDIS_URL` Reference를 제거하고
   재배포한다. 앱은 dump 제한 fail-open(로컬 TTL 랭킹/세션)으로 돌아간다.
   `REDIS_URL`이 남아 있는데 Redis만 죽은 경우는 dump가 429 fail-closed이므로
   URL 제거 또는 Redis 복구가 필요하다.
...
- [ ] Redis 중지 smoke로 dump fail-closed(429)와 `rateLimitBypassed=true` 로그 확인
- [ ] Redis URL 제거 후 dump fail-open(로컬 개발 모드)으로 복귀하는지 확인
