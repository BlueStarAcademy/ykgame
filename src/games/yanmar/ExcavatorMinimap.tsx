...
export function ExcavatorMinimap({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      ctx.fillStyle = shell;
      ctx.beginPath();
      ctx.roundRect(0, 0, size, size, shellRadius);
      ctx.fill();
      ...
      for (let i = 1; i < 4; i++) {
        ...
        ctx.beginPath();
        ctx.moveTo(p + 0.5, pad);
        ctx.lineTo(p + 0.5, size - pad);
        ctx.moveTo(pad, p + 0.5);
        ctx.lineTo(size - pad, p + 0.5);
        ctx.stroke();
      }
      ...
      const drawZoneLabel = (label: string, x: number, y: number, color: string) => {
        ctx.save();
        ctx.font = `900 ${Math.max(8, Math.round(10 * labelScale))}px "Malgun Gothic", "Apple SD Gothic Neo", "Noto Sans KR", sans-serif`;
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.lineWidth = Math.max(2, 3 * labelScale);
      ...
      for (const zone of digZones) {
        ...
        ctx.fillStyle = "rgba(255,143,0,0.32)";
        ctx.beginPath();
        ctx.arc(dig.px, dig.py, Math.max(digR, 4.5 * (size / DEFAULT_DISPLAY_SIZE)), 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.arc(dig.px, dig.py, 3.5 * (size / DEFAULT_DISPLAY_SIZE), 0, Math.PI * 2);
        ctx.fill();
      ...
      ctx.beginPath();
      ctx.arc(dump.px, dump.py, Math.max(dumpR, 3.5 * (size / DEFAULT_DISPLAY_SIZE)), 0, Math.PI * 2);
      ctx.fill();
      ...
      ctx.fillRect(dump.px - dumpMark, dump.py - dumpMark, dumpMark * 2, dumpMark * 2);
      ...
      if (wp) {
        ...
        ctx.beginPath();
        ctx.arc(goal.px, goal.py, 6.5 * (size / DEFAULT_DISPLAY_SIZE), 0, Math.PI * 2);
        ctx.fill();
      ...
      const nx = -dirY;
      const ny = dirX;
      ctx.shadowColor = "rgba(239,83,80,0.55)";
      ctx.shadowBlur = 4;
      ...
      ctx.beginPath();
      ctx.arc(px, py, 2 * markerScale, 0, Math.PI * 2);
      ctx.fill();
